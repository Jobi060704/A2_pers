...
public:
    ...
    int calculateChange(int owed, std::map <int,Coin> &coins);

    // displays the coins
    std::string displayCoins(std::map<int, Coin> &coins);
    // resets the coins
    std::string resetCoins(int value, std::map<int, Coin> &coins);
};
